...
def predict_and_save_csv_per_block(model, data_root, device, seq_len=5):
    model.eval()
    os.makedirs("predictions_trans2", exist_ok=True)
    ...
    for p in patients:
        ...
        try:
            ...
            save_path = f"predictions_trans2/{p}_preds.csv"
...
def load_data_dependent(data_root, seq_len=5, batch_size=8):
    datasets = []

    print(f"📂 Đang load tất cả block từ thư mục: {data_root}")
    ...
    if not datasets:
        raise RuntimeError("❌ Không load được block nào!")
...
if __name__ == "__main__":
    ...
    train_loader, val_loader, test_loader = load_data_dependent(data_path, seq_len=5, batch_size=8)
